 ...
 *   `agent` selects a role from ~/.pi/agent/agents/*.md, which supplies the
 *   child's prompt preamble and its default model, effort, and tool allowlist.
...
export default function (pi: ExtensionAPI) {
  const { roles: agentRoles, rejected: rejectedRoles } = loadAgentRoles(
    getAgentDir(),
  );
  const roleByName = new Map(agentRoles.map((role) => [role.name, role]));
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    if (rejectedRoles.length > 0 && ctx.hasUI) {
      ctx.ui.notify(
        `Skipped ${rejectedRoles.length} agent role file(s): ${rejectedRoles
          .map((bad) => `${bad.file} (${bad.reason})`)
          .join("; ")}`,
        "warning",
      );
    }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      agent: Type.Optional(
        Type.String({
          description: buildAgentParameterDescription(agentRoles),
        }),
      ),
    }),
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const role = agentName ? roleByName.get(agentName) : undefined;
      if (agentName && !role) {
        throw new Error(
          `Unknown agent "${agentName}". Available: ${agentRoles.map((r) => r.name).join(", ") || "none"}.`,
        );
      }
      // Only the pi harness can enforce a tool allowlist. Running a read-only
      // role with full write access because another harness ignored its tools
      // is worse than refusing to start.
      if (role?.tools && harness !== "pi") {
        throw new Error(
          `Agent "${role.name}" restricts tools to ${role.tools.join(", ")}, ` +
            `which only the pi harness can enforce. Use harness "pi", or drop ` +
            `the tools: key from the role file.`,
        );
      }
      ...
      const snap = await runTool(
        ...
        manager.spawn(harness, {
          prompt: role ? applyRole(role, params.prompt) : params.prompt,
          title,
          cwd,
