...
/**
 * Where a subagent is working, shortened for a dashboard row. Subagents can be
 * spawned into any directory, so this is often the only thing distinguishing
 * two children with the same role.
 */
export function formatCwd(cwd: string, home = homedir()): string {
  ...
  return segments.length <= 3 ? short : `…/${segments.slice(-2).join("/")}`;
}
...
class SubagentDashboard implements Component {
  ...
  private renderRows(
    ...
    for (let i = 0; i < visible.length; i++) {
      ...
      const left = ` ${marker} ${statusGlyph(snap, theme)} ${title} ${theme.fg("dim", snap.id)} ${theme.fg("dim", formatCwd(snap.cwd))}`;
...
class TakeoverView implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const header =
      ...
      theme.fg("dim", ` · ${formatCwd(snap.cwd)}`) +
