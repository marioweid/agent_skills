...
export const SUBAGENT_SPAWN_PROMPT_GUIDELINES = [
  "Work in the current session by default. Delegate only a bounded task with a concrete benefit over the extra context and handoff cost; never delegate when the user asks you to work alone.",
...
/** Renders the `agent` enum description from the roles found on disk. */
export function buildAgentParameterDescription(
  roles: readonly { name: string; description: string }[],
) {
  if (roles.length === 0) {
    return "Optional specialist role. No role files are present in ~/.pi/agent/agents, so leave this unset.";
  }
  const lines = roles.map(
    (role) => `${role.name}: ${role.description.split(". ")[0]!.slice(0, 110)}`,
  );
  return `Optional specialist role for this subagent. It supplies the child's prompt preamble plus its default model, reasoning effort, and (on the pi harness) its tool allowlist; omit for a plain agent driven only by your prompt. Your prompt is still required — it is the task the role performs. Roles: ${lines.join(" | ")}.`;
}
