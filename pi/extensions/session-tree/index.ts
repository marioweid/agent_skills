 ...
 * between.
 *
 * `/sessions` (or ctrl+g) opens a directory → session tree. Enter switches
 * this window into the selected session. Sessions another pi window currently
 * holds open are shown but not enterable: a transcript has one writer.
 *
 * With `openAtStartup`, the tree is what you see when pi opens, so a window
 * starts as a session picker rather than an empty prompt.
 *
 * Only slash commands get a session-control context, so the startup view and
 * the shortcut both route through `/sessions` rather than duplicating it.
 */
...
export default function sessionTree(pi: ExtensionAPI) {
  ...
  /** Rows hidden with ctrl+x. Session-scoped: ctrl+r and restarting restore them. */
  ...
  pi.registerShortcut(SHORTCUT, {
    description: "Browse and switch pi sessions",
    handler: openTree,
  });
  ...
  pi.registerCommand(COMMAND, {
    ...
    handler: async (_args, ctx) => {
      ...
      const outcome = await openSessionTree(ctx, store, await listSessions(), hidden);
      if (outcome.action !== "switch") return;
