...
> Pydantic AI takes the model as a `"provider:model"` string, so it's swappable across providers (openai, anthropic, google, groq, …). Examples in this skill use `"<provider>:<model>"` as a placeholder — replace it with a real prefix and model, e.g. one from the [Model Provider Prefixes](./references/ARCHITECTURE.md#model-provider-prefixes) table.
...
my_agent = Agent('<provider>:<model>', instructions='...')
...
**Quick reference — model string format:** `"provider:model-name"` — examples in this skill use the `"<provider>:<model>"` placeholder; see [Model Provider Prefixes](./references/ARCHITECTURE.md#model-provider-prefixes) for concrete prefixes.
...
- **Model strings need the provider prefix**: `'<provider>:<model>'` not `'<model>'`. Without the prefix, Pydantic AI can't resolve the provider.
