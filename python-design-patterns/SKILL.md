...
### 5. Dependency Injection & Context Objects

Pass a class its collaborators through the constructor — hand-wired, no framework or service locator. When many call sites need the same app- or request-scoped singletons, bundle them into one typed context object (a `@dataclass`) built once and injected via a single accessor, not many loose global accessors. Explicit dependencies stay testable: substitute a fake in the constructor.

### 6. Thin Adapters at the Edges

Entry points — route handlers, CLI commands, message consumers — resolve a service and delegate. Orchestration and business logic live in the service that owns the flow, never in the handler. The class-vs-function split is statefulness, not locality: a helper used by one class still stays a free function if it holds no state.
...
11. **Context objects over loose accessors** - One typed container of shared singletons, built once and injected
12. **Thin edges** - Handlers/commands delegate to services; no orchestration at the boundary
