...
Service-oriented by default: cohesive behavior lives in classes, dependencies are injected, entry points stay thin. Don't force OOP where a function is clearer — split by what the code actually needs.

- **State decides class vs. function** - Put cohesive state + behavior in a class; keep stateless transforms as free/module functions. "Used in only one place" is not a reason to make a helper a method — locality doesn't decide, state does. Never wrap a stateless helper as a static method just to file it under a class (a static method that touches no instance state is a function with extra ceremony). A class is not the default unit of code.
- **Inject dependencies; pass shared state as one context object** - Classes receive their collaborators through the constructor, hand-wired — no DI framework or service locator. When many call sites need the same app- or request-scoped singletons, bundle them into a single typed context object built once and handed in through one accessor, instead of many loose global accessors.
- **Entry points are thin adapters** - Route handlers, CLI commands, and message consumers resolve the relevant service and delegate; they hold no orchestration or business logic. Orchestration lives in a service that owns the flow.
